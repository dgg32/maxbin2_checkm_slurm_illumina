...
R1=$(find $1 -maxdepth 2 -regex '\S+fastq'  -not -name "*trimmed*" | grep "R1" | head -n $SLURM_ARRAY_TASK_ID | tail -n 1)
...
if [ ! -f $R1_no_carp ];
then
python filter_fasta_multiprocess.py './taboo.txt' $R1trim
fi
...
checkm  lineage_wf -t 8 -x fasta $maxbinoutfolder $checkmfolder
fi
